...
from exercise_19_passwd_to_dict.exercise_19_passwd_to_dict import passwd_to_dict
from exercise_19_passwd_to_dict.exercise_19_shell_dict import shells_users
...
def test_shells_users(complex_passwd):
    d = shells_users(complex_passwd)
    assert len(d) == 3
    assert d['/bin/bash'] == ['root']
    assert d['/usr/sbin/nologin'] == ['daemon', 'bin', 'sys', 'games', 'man', 'lp', 'mail']
    assert d['/bin/sync'] == ['sync']
